    ...
    <el-row justify="center" type="flex">
      <el-col :span="computedSpan">
        ...
        >
          ...
          <slot />
          <el-row :gutter="20">
            <slot :formData="formData" :formDesc="orderedFormDesc" :formErrorObj="formErrorObj" :props="$props" name="form-content">
                    ...
                    >
                      <template slot="label" v-if="formItem.note">
                        <span>
                          ...
                          <el-popover placement="top-start" width="200" trigger="hover" :content="formItem.note">
                      ...
                      <slot
                        ...
                        :options="formItem._options"
                      >
                        <!-- disabled属性添加,作用于组件的全局配置, 穿透到引用的组件中,目前为了解决,
                          上传文件组件EleUploadFile,全局配置项disabled=true时候,还会出现删除按钮的bug   by bing-->
                        <component
            ...
            <el-col class="ele-form-btns" v-if="btns.length">
              <el-form-item :label-width="inline ? '10px' : null">
                ...
                <slot :btns="btns" name="form-btn">
                  <el-button :key="index" @click="btn.click" v-bind="btn.attrs" v-for="(btn, index) of btns">
                    {{ btn.text }}
                  </el-button>
                </slot>
              </el-form-item>
...
export default {
  ...
  mixins: [responsiveMixin, localeMixin, searchArrKey],
  model: {
    prop: 'formData',
    event: 'input'
  },
  ...
  computed: {
    isMock() {
      return this.mock || Object.values(this.formDescData).some((item) => item.mock)
    },
    // 按钮
    btns() {
    ...
    computedSubmitBtnText() {
      if (is(this.submitBtnText, 'String')) {
        ...
        return this.inline ? t('ele-form.submitBtnTextInline') : t('ele-form.submitBtnText')
      }
    },
    // 标签宽度(数字和字符串两种处理)
    computedLabelWidth() {
    ...
    computedRules() {
      return this.formDescKeys.reduce((rules, field) => {
        ...
        let item = this.formDescData[field]
        // 如果采用required, 则判断已有的规则有无, 如果没有, 则添加
        if (this.formDescData[field].required && !rules[field].some((rule) => rule.required)) {
          rules[field].push({
            required: true,
            message: this.formDescData[field]._label + t('ele-form.required')
          })
        }
        this.verifyFormat(item, (data) => {
          rules[field].push(data)
        })
        return rules
      }, {})
    },
  ...
  watch: {
    ...
    formDesc: {
      handler(formDesc) {
        ...
        this.formDescData = Object.assign({}, oldFormDescData, cloneDeep(formDesc))
      },
      immediate: true,
      deep: true
    },
  ...
  methods: {
    // input回车
    keyUp13(field, val) {
      if (this.formDescData[field].on && this.formDescData[field].on['keyUp13']) {
    ...
    getDynamicAttr(attr, field) {
      return typeof attr === 'function' ? this.getFunctionAttr(attr, field) : attr
    },
    // 检测联动
    checkLinkage() {
      if (this.checkVifFn) {
        ...
        this.checkLinkageFn = throttle(300, () => {
          ...
          Object.keys(formDescData).forEach((field) => {
            ...
            if (typeof formItem.type === 'function') {
              type = this.getComponentName(this.getFunctionAttr(formItem.type, field))
              if (formItem._type && formItem._type !== type) {
                // 获取此类型的以前值
                const newVal = formItem._oldValue['type-' + type] || null
                // 保存现在的数据作为老数据
                this.formDescData[field]._oldValue['type-' + formItem._type] = formData[field]
            ...
            const optionsLinkageFields = castArray(this.getDynamicAttr(formItem.optionsLinkageFields, field))
    ...
    setDefaultvalue(formItem, field) {
      let defaultValue = typeof formItem.default === 'function' ? formItem.default(this.formData) : formItem.default
      // 默认值不为空  & (值为空 || 老值和当前值)
    ...
    shouldRequest(field) {
      ...
      return formItem._optionsLinkageFields.some((field) => this.formData[field] !== this.oldFormData[field])
    },
      ...
      if (options) {
        if (options instanceof Array) {
          ...
          if (typeof options === 'string') {
            throw new TypeError(`options值为: ${options}, 为字符串, 但是未配置options-fn参数, `)
          } else {
            // 其他报错
            throw new TypeError('options的类型不正确, options及options请求结果类型可为: 字符串数组, 对象数组, 函数和Promise或者URL地址, 当前值为: ' + options)
          }
        }
      } else {
    ...
    setOptions(options, field) {
      ...
      const oldOptionsValues = (formItem._options || []).map((item) => item.value).join(',')
      // 改变prop为规定的prop
    ...
    // 验证所有组件的内置验证方法
    validateComponent() {
      const validators = this.formDescKeys.map((key) => (this.$refs[key] && this.$refs[key][0] ? this.$refs[key][0].validate : null)).filter(Boolean)
    ...
    processError(errObj) {
      ...
      try {
        const messageArr = Object.keys(errObj).reduce((acc, key) => {
          ...
          const label = formItem && formItem._label ? formItem._label + ': ' : key + ': '
    ...
    async handleSubmitForm(actionType = 'button') {
      ...
      try {
        ...
        //inline 为 false时转化一下attrs里面含有searchArrKey属性的值
        if (!this.inline) {
          data = this.getArrtoMap(data)
        }
...
.ele-form .is-justify-center .el-row {
  display: flex;
  flex-wrap: wrap;
}
