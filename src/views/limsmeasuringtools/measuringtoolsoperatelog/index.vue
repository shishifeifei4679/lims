  ...
  <div>
    <div>
      <ele-page
        ...
        :column="column"
        :getList="getList"
        uniqueId="measuringtoolsoperatelog"
      >
      </ele-page>
    </div>
...
} from '@/api/limsmeasuringtools/measuringtoolsoperatelog'
import {
    getDict // 字典查询
} from '@/api/common';
import uploadNoAuto from "@/components/upload-no-auto";
export default {
  name: "limsMeasuringToolsOperateLog",
  components:{
    uploadNoAuto
  },
  data() {
    return {
      ...
      column: [
        ...
        {
          prop: "orderNo",
          label: $i18n.t('limsMeasuringToolsOperateLog.orderNo'),  // 申请编码
          width: '120',
        ...
        },
        {
          ...
          label: $i18n.t('limsMeasuringToolsOperateLog.measuringToolCode'),  // 量检具编码
          width: '120',
          showOverflowTooltip: true,
          showColumnSearch: true,
          showColumnSearchConfig: { // 非必填 对象{} 不设置，就是默认模糊搜索
        ...
        },
        {
          ...
          label: $i18n.t('limsMeasuringToolsOperateLog.measuringToolName'),  // 量检具名称
          width: '150',
          showOverflowTooltip: true,
          showColumnSearch: true,
          showColumnSearchConfig: { // 非必填 对象{} 不设置，就是默认模糊搜索
        ...
        {
          prop: "operateType",
          label: $i18n.t('limsMeasuringToolsOperateLog.operateType'),  // 操作类型(字典：lims_operate_type)【0：量检具登记 1：启用 2：封存 3：报废 4：停用 5：丢失】
          width: '120',
          showOverflowTooltip: true,
          showColumnSearch: true,
          ...
          option: async (data) => {
            const list = await getDict("lims_operate_type");
            return list.data.result;
          },
        },
        {
          prop: "operateContent",
          label: $i18n.t('limsMeasuringToolsOperateLog.operateContent'),  // 情况说明
          width: '160',
          showOverflowTooltip: true,
          showColumnSearch: true,
          showColumnSearchConfig: { // 非必填 对象{} 不设置，就是默认模糊搜索
...
//          },
        ...
        {
          prop: "createByName",
          label: $i18n.t('limsMeasuringToolsOperateLog.createByName'),  // 创建人
          width: '120',
          showOverflowTooltip: true,
          showColumnSearch: true,
          showColumnSearchConfig: { // 非必填 对象{} 不设置，就是默认模糊搜索
        ...
        {
          prop: "createTime",
          label: $i18n.t('limsMeasuringToolsOperateLog.createTime'),  // 创建时间
          width: 160,
          showOverflowTooltip: true,
          showColumnSearch: true,
          showColumnSearchConfig: {
    ...
    rowDetail(id) {
      rowDetail(id)
        .then((res) => {
          ...
          const contractPaths = contractPath
            ? contractPath.split(',').map((val) => {
                const arr = val.split("/")
                return {
                  url: val,
